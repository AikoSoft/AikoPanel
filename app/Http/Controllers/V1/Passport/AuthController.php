...
class AuthController extends Controller
{
    public function loginWithMailLink(Request $request)
    {
        ...
        if (config('aikopanel.app_url')) {
            $link = config('aikopanel.app_url') . $redirect;
        } else {
            $link = url($redirect);
        }
    ...
    public function register(AuthRegister $request)
    {
        if ((int)config('aikopanel.register_limit_by_ip_enable', 0)) {
            ...
            if ((int)$registerCountByIP >= (int)config('aikopanel.register_limit_count', 3)) {
                abort(500, __('Register frequently, please try again after :minute minute', [
                    'minute' => config('aikopanel.register_limit_expire', 60)
                ]));
            }
        }
        if ((int)config('aikopanel.recaptcha_enable', 0)) {
            $recaptcha = new ReCaptcha(config('aikopanel.recaptcha_key'));
            $recaptchaResp = $recaptcha->verify($request->input('recaptcha_data'));
            if (!$recaptchaResp->isSuccess()) {
                abort(500, __('Invalid code is incorrect'));
            }
        }
        if ((int)config('aikopanel.email_whitelist_enable', 0)) {
            if (!Helper::emailSuffixVerify(
                $request->input('email'),
                config('aikopanel.email_whitelist_suffix', Dict::EMAIL_WHITELIST_SUFFIX_DEFAULT))
            ) {
        ...
        if ((int)config('aikopanel.try_out_plan_id', 0)) {
            $plan = Plan::find(config('aikopanel.try_out_plan_id'));
            if ($plan) {
        ...
        if ((int)config('aikopanel.register_limit_by_ip_enable', 0)) {
            Cache::put(
                ...
                (int)config('aikopanel.register_limit_expire', 60) * 60
            );
        }
    ...
    public function login(AuthLogin $request)
    {
        ...
        if ((int)config('aikopanel.password_limit_enable', 1)) {
            ...
            if ($passwordErrorCount >= (int)config('aikopanel.password_limit_count', 5)) {
                abort(500, __('There are too many password errors, please try again after :minute minutes.', [
                    'minute' => config('aikopanel.password_limit_expire', 60)
                ]));
            }
        }
        ...
        if (!Helper::multiPasswordVerify(
            ...
            if ((int)config('aikopanel.password_limit_enable')) {
                Cache::put(
                    ...
                    60 * (int)config('aikopanel.password_limit_expire', 60)
                );
            }
            abort(500, __('Incorrect email or password'));
        }
    ...
    public function token2Login(Request $request)
    {
        if ($request->input('token')) {
            ...
            if (config('aikopanel.app_url')) {
                $location = config('aikopanel.app_url') . $redirect;
            } else {
