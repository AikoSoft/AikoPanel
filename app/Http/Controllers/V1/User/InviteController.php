...
class InviteController extends Controller
{
    ...
    {
        if (InviteCode::where('user_id', $request->user['id'])->where('status', 0)->count() >= config('aikopanel.invite_gen_limit', 5)) {
    ...
    public function fetch(Request $request)
    {
        $codes = InviteCode::where('user_id', $request->user['id'])
            ->where('status', 0)
            ->get();
        $commission_rate = config('aikopanel.invite_commission', 10);
        ...
        if (config('aikopanel.commission_distribution_enable', 0)) {
            $uncheck_commission_balance = $uncheck_commission_balance * (config('aikopanel.commission_distribution_l1') / 100);
        }
        $stat = [
            //已注册用户数
