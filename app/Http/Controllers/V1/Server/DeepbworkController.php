...
class DeepbworkController extends Controller
{
    ...
    public function user(Request $request)
    {
        ...
        foreach ($users as $user) {
            $user->v2ray_user = [
                "uuid" => $user->uuid,
                "email" => sprintf("%s@aikopanel.user", $user->uuid),
                "alter_id" => 0,
                "level" => 0,
            ];
            unset($user['uuid']);
...
//         Log::info('serverSubmitData:' . $request->input('node_id') . ':' . request()->getContent() ?: json_encode($_POST));
    ...
    private function getV2RayConfig(int $nodeId, int $localPort)
    {
        $server = ServerVmess::find($nodeId);
        if (!$server) {
            abort(500, __('Node does not exist'));
        }
        $json = json_decode(self::V2RAY_CONFIG);
        $json->log->loglevel = (int)config('aikopanel.server_log_enable') ? 'debug' : 'none';
    ...
    {
        $domainRules = array_filter(explode(PHP_EOL, config('aikopanel.server_v2ray_domain')));
        $protocolRules = array_filter(explode(PHP_EOL, config('aikopanel.server_v2ray_protocol')));
