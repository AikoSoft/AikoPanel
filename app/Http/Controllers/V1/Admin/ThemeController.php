...
class ThemeController extends Controller
{
    ...
    public function getThemes()
    {
        ...
        return response([
            'data' => [
                'themes' => $themeConfigs,
                'active' => config('aikopanel.frontend_theme', 'aikopanel')
            ]
        ]);
    }
    ...
    public function saveThemeConfig(Request $request)
    {
        ...
        if (!$payload['config'] || !is_array($payload['config'])) abort(500, __('Incorrect parameters'));
        $themeConfigFile = public_path("theme/{$payload['name']}/config.json");
        if (!File::exists($themeConfigFile)) abort(500, __('Theme configuration file does not exist'));
        $themeConfig = json_decode(File::get($themeConfigFile), true);
        if (!isset($themeConfig['configs']) || !is_array($themeConfig)) abort(500, __('Incorrect theme configuration file'));
