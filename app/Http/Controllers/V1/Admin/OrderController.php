...
class OrderController extends Controller
{
    ...
    public function paid(Request $request)
    {
        ...
        if ($order->status !== 0) abort(500, __('Only orders that are pending payment can be operated'));
    ...
    public function assign(OrderAssign $request)
    {
        ...
        if (!$plan) {
            abort(500, __('Plan does not exist'));
        }

        $userService = new UserService();
        if ($userService->isNotCompleteOrderByUserId($user->id)) {
            abort(500, __('There is an unpaid order'));
        }
        ...
        if (!$order->save()) {
            DB::rollback();
            abort(500, __('Order creation failed'));
        }
