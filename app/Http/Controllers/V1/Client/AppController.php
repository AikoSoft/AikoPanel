    ...
    public function getVersion(Request $request)
    {
        if (strpos($request->header('user-agent'), 'tidalab/4.0.0') !== false
            ...
            if (strpos($request->header('user-agent'), 'Win64') !== false) {
                return response([
                    'data' => [
                        'version' => config('aikopanel.windows_version'),
                        'download_url' => config('aikopanel.windows_download_url')
                    ]
                ]);
            } else {
                return response([
                    'data' => [
                        'version' => config('aikopanel.macos_version'),
                        'download_url' => config('aikopanel.macos_download_url')
                    ]
                ]);
            }
            return;
