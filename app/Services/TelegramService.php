...
class TelegramService {
    ...
    {
        $this->api = 'https://api.telegram.org/bot' . config('aikopanel.telegram_bot_token', $token) . '/';
    }
    ...
    private function request(string $method, array $params = [])
    {
        ...
        if (!$response->ok) {
            abort(500, __('Error from TG:') . $response->description);
        }
        return $response;
    }
    ...
    {
        if (!config('aikopanel.telegram_bot_enable', 0)) return;
